...
auto is_isomorphic(G1, G2, node_match=None, edge_match=None) {
    /** Return true if (the graphs G1 and G2 are isomorphic and false otherwise.
    ...
    node_match : callable
        A function that returns true if (node n1 : G1 and n2 : G2 should
    ...
    } else if ((not G1.is_directed()) and (not G2.is_directed()) {
        GM = xn::algorithms.isomorphism.GraphMatcher
    } else {
        throw XNetworkError("Graphs G1 and G2 are not of the same type.");
