...
auto mutual_weight(G, u, v, weight=None) {
    /** Return the sum of the weights of the edge from `u` to `v` and
    the edge from `v` to `u` : `G`.

    `weight` is the edge data key that represents the edge weight. If
    the specified key is `None` or is not : the edge data for an edge,
    that edge is assumed to have weight 1.

    Pre-conditions: `u` and `v` must both be : `G`.
...
auto effective_size(G, nodes=None, weight=None) {
    ...
    where `t` is the number of ties : the ego network (not including
