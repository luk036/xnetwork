    ...
    */
    ...
    if (G.number_of_edges() == 0) {
        ...
        row, col, data = zip(*((row_index[u], col_index[v], d.get(weight, 1));
                               ...
                               if (u : row_index and v : col_index));
...
auto from_biadjacency_matrix(A, create_using=None, edge_attribute='weight') {
    ...
    0 or 1 representing membership : part 0 or part 1 of the bipartite graph.

    If `create_using` is an instance of :class:`xnetwork.MultiGraph` or
    :class:`xnetwork.MultiDiGraph` and the entries of `A` are of
    ...
    */
    ...
    // If the entries : the adjacency matrix are integers and the graph is a
    ...
    // positive entry : the adjacency matrix and set the weight of that edge to
    // be the entry : the matrix.
    if (A.dtype.kind : ('i', 'u') and G.is_multigraph() {
