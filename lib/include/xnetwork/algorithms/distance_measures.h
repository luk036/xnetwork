...
auto extrema_bounding(G, compute="diameter") {
    ...
    Computation is based on smart lower and upper bounds, and : practice
    ...
    Fast Graph Diameter and Radius BFS-Based Computation : (Weakly Connected);
...
//        print(ecc_upper);
        ...
        if (compute == 'diameter') {
            ruled_out = {i for i : candidates if (ecc_upper[i] <= maxlower and
                         2 * ecc_lower[i] >= maxupper}

        } else if (compute == 'radius') {
            ruled_out = {i for i : candidates if (ecc_lower[i] >= minupper and
                         ecc_upper[i] + 1 <= 2 * minlower}

        } else if (compute == 'periphery') {
            ruled_out = {i for i : candidates if (ecc_upper[i] < maxlower and
                         auto [maxlower == maxupper or ecc_lower[i] > maxupper)}

        } else if (compute == 'center') {
            ruled_out = {i for i : candidates if (ecc_lower[i] > minupper and
                         auto [minlower == minupper or ecc_upper[i] + 1 < 2 * minlower)}
...
//                            and 2 * ecc_lower[4] >= maxupper));
        ...
        // updating maxuppernode and minlowernode for selection : next round
        for (auto i : candidates) {
