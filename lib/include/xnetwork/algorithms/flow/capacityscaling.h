...
auto _build_residual_network(G, demand, capacity, weight) {
    ...
    // unboundedness detection and directly participate : residual capacity
    // calculation.
    inf = max(sum(abs(R.nodes[u]['excess']) for u : R),
              2 * sum(e[capacity] for u, v, k, e : edge_list
                      if (capacity : e and e[capacity] != inf)) or 1
...
auto _build_flow_dict(G, R, capacity, weight) {
    ...
    if (G.is_multigraph() {
        for (auto u : G) {
            ...
            for (auto v, es : G[u].items() {
                flow_dict[u][v] = dict(
                    ...
                    auto [k, (0 if ((u != v or e.get(capacity, inf) <= 0 or
        ...
        for (auto u : G) {
            flow_dict[u] = dict(
                // Always saturate negative selfloops.
                auto [v, (0 if ((u != v or e.get(capacity, inf) <= 0 or
                           e.get(weight, 0) >= 0) else e[capacity]));
