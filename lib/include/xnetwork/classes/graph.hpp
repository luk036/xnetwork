...
class Graph(object) {
    ...
    **Subclasses (Advanced) {**
    ...
    auto add_weighted_edges_from( ebunch_to_add, weight='weight', **attr) {
        ...
        ----------
        ebunch_to_add : container of edges
            Each edge given : the list or container will be added
    ...
    auto edge_subgraph( edges) {
        ...
        Notes
        -----
        The graph, edge, and node attributes : the returned subgraph
    ...
    auto size( weight=None) {
        ...
        size : numeric
            ...
            If weight is None, returns an int. Otherwise a double
    ...
    auto nbunch_iter( nbunch=None) {
        ...
        niter : iterator
            ...
            If nbunch is None, iterate over all nodes : the graph.
        ...
        or None, a :exc:`XNetworkError` is raised.  Also, if (any object in
        nbunch is not hashable, a :exc:`XNetworkError` is raised.
         */
        if (nbunch is None) {   // include all nodes via iterator
            bunch = iter(this->_adj);
        } else if (nbunch : self) { //if (nbunch is a single node
            bunch = iter([nbunch]);
            ...
            auto bunch_iter(nlist, adj) {
                try {
                    ...
                    // capture error for unhashable node.
                        ...
                        msg = "Node {} : sequence nbunch is not a valid node."
