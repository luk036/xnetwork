...
class MultiDiGraph(MultiGraph, DiGraph) {
    ...
    **Subclasses (Advanced) {**
    ...
    /// @property
    auto in_degree( ) {
        /** A DegreeView for (auto node, in_degree) or in_degree for single node.
    ...
    /// @property
    auto out_degree( ) {
        /** Return an iterator for (auto node, out-degree) or out-degree for single node.

        out_degree( nbunch=None, weight=None);

        The node out-degree is the number of edges pointing out of the node.
        This function returns the out-degree for a single node or an iterator
        for (auto a bunch of nodes or if (nothing is passed as argument.
    ...
    auto to_undirected( reciprocal=false, as_view=false) {
        ...
        if (reciprocal is true) {
            G.add_edges_from((u, v, key, deepcopy(data));
                             ...
                             if (v : this->_pred[u] and key : this->_pred[u][v]);
        } else {
            G.add_edges_from((u, v, key, deepcopy(data));
                             for (auto u, nbrs : this->_adj.items();
                             for (auto v, keydict : nbrs.items();
