...
#include <xnetwork/convert.hpp>using namespace convert;
...
class DiGraph(Graph) {
    ...
    **Subclasses (Advanced) {**
    ...
    auto has_successor( u, v) {
         ...
         */
        return (u : this->_succ and v : this->_succ[u]);
    ...
    auto has_predecessor( u, v) {
        ...
        This is true if (graph has the edge u<-v.
         */
        return (u : this->_pred and v : this->_pred[u]);
    ...
    /// @property
    auto in_degree( ) {
        /** An InDegreeView for (auto node, in_degree) or in_degree for single node.
    ...
    auto to_undirected( reciprocal=false, as_view=false) {
        ...
        G : Graph
            ...
            is : the digraph.  If both edges exist : digraph and
        ...
        Notes
        -----
        If edges : both directions (u, v) and (v, u) exist : the
