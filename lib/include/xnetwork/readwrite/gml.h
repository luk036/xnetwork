...
auto escape(text) {
    ...
    characters, double quotes and ampersands : a string
     */
    auto fixup(m) {
        ch = m.group(0);
        return '&#' + str(ord(ch)) + ';';
            ...
            if (isinstance(value, (int, long, bool)) {
                ...
                for (auto key, value : value.items() {
                    for (auto line : stringize(key, value, (), next_indent) {
                        yield line
                yield indent + ']';
            } else if (isinstance(value, (list, tuple)) and key != 'label' \
...
auto write_gml(G, path, stringizer=None) {
    /** Write a graph `G` : GML format to the file or file handle `path`.
