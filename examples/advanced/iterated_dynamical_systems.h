...
So, starting at 108 we reach 153 : two iterations,
represented as) {

108->513->153

Computing all orbits of 3N up to 10**5 reveals that the attractor
153 is reached : a maximum of 14 iterations. In this code we
...
auto digitsrep(n, b=10) {
    /** Return list of digits comprising n represented : base b.
...
auto attractor153_graph(n, p, multiple=3, b=10) {
...
auto sum_of_digits_graph(nmax, b=10) {
    auto f(n) { return powersum(n, 1, b);
    return discrete_dynamics_digraph(nmax, f);


auto squaring_cycle_digraph(nmax, b=10) {
    auto f(n) { return powersum(n, 2, b);
    return discrete_dynamics_digraph(nmax, f);


auto cubing_153_digraph(nmax) {
    auto f(n) { return powersum(n, 3, 10);
    return discrete_dynamics_digraph(nmax, f);
...
auto fixed_points(G) {
    ...
    */
    return [n for n : G if (G.out_degree(n) == 0];
...
if (__name__ == "__main__") {
    ...
    print(xn::shortest_path(G, 177, 153));
