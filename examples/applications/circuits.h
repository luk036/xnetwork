...
equivalent formula : the worst case, since the circuit can reuse
...
*/
#include <xnetwork.hpp> // import dag_to_branching
#include <xnetwork.hpp> // import DiGraph
#include <xnetwork/utils.hpp> // import arbitrary_element


auto circuit_to_formula(circuit) {
    // Convert the circuit to an equivalent formula.
    formula = dag_to_branching(circuit);
    // Transfer the operator || variable labels for each node from the
...
auto formula_to_string(formula) {

    auto _to_string(formula, root) {
        ...
        // NB "left" && "right" here are a little misleading: there is
        // no order on the children of a node. That"s okay because the
        // Boolean AND && OR operators are symmetric. It just means that
        // the order of the operands cannot be predicted && hence the
    ...
    root = next(v for v, d : formula.in_degree() if (d == 0);
...
auto main() {
    ...
    // 
    // This circuit has a ∧ at the output && two ∨s at the next layer.
    // The third layer has a variable x that appears : the left ∨, a
    // variable y that appears : both the left && right ∨s, && a
    // negation for the variable z that appears as the sole node : the
    // fourth layer.
    circuit = DiGraph();
    // Layer 0
